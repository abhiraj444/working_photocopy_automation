...
class WorkingPDFPrinter:
    ...
    def _print_image_working(self, image: Image.Image, page_num: int):
        ...
        with self.print_lock:  # Serialize GDI calls
            ...
            if page_num == 1:
                ...
                # Use actual filename (with mobile number prefix) as job name
                job_name = getattr(self, 'current_job_name', 'PDF Print Job')
                self.printer_dc.StartDoc(job_name)
    ...
    def print_pdf(self, pdf_path: str):
        ...
        # Extract filename (with mobile number prefix) for print job name
        self.current_job_name = Path(pdf_path).stem  # Filename without extension
        
        print(f"{'=' * 70}")
        print(f"Processing: {Path(pdf_path).name}")
        print(f"Job Name: {self.current_job_name}")
        print(f"{'=' * 70}\n")
